...
$txt['mboards_description_desc'] = 'Небольшое описание раздела. HTML теги не допускаются, можно использовать BBC теги: {allowed_tags} ';
$txt['mboards_cat_description_desc'] = 'Небольшое описание категории. HTML теги не допускаются, можно использовать BBC теги: {allowed_tags} ';
...
$txt['boardsaccess_option_desc'] = 'Для каждого права доступа можно установить \'Разрешить\' (+), \'Отклонить\' (-), или <span class="alert">\'Запретить\' (X)</span>.<br><br>
Если запретить права, любой пользователь, находящийся в этой группе (даже модератор), лишится этих прав.<br>
Поэтому, следует пользоваться запретом прав с осторожностью, только когда это <strong>действительно необходимо</strong>. Отклонение прав, с другой стороны, запрещает, если не перекрывается другим разрешающим правом. ';
